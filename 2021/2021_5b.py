...
def diagonalmove(startx, endx, starty, endy):
    if startx > endx:
        movex = -1
    else:
        movex = 1
    ...
    differencex = abs(startx - endx)

    for i in range(differencex+1):
        map[starty + movey*i][startx +movex *i] = str(int(map[starty + movey*i][startx +movex *i])+1)
...
for i in data: #loop thorugh each set of 2 coordinate
    if i[0][1] == i[1][1]: # same y values
        ...
        diagonalmove(i[0][0], i[1][0], i[0][1],i[1][1])
